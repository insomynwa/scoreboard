...
class Gamedraw_Model_Class{
    ...
    public function table_data(){
        ...
        $query =
        "SELECT gd.gamedraw_id, gd.gamedraw_num, bs.bowstyle_name, gm.gamemode_name, ta.team_name contestant_a_name, tb.team_name contestant_b_name FROM {$this->table_name} gd
        LEFT JOIN bowstyles bs ON bs.bowstyle_id=gd.bowstyle_id
        LEFT JOIN gamemode gm ON gm.gamemode_id=gd.gamemode_id
        LEFT JOIN team ta ON ta.team_id=gd.contestant_a_id
        LEFT JOIN team tb ON tb.team_id=gd.contestant_b_id
        WHERE gd.gamemode_id=1
        UNION
        SELECT gd.gamedraw_id, gd.gamedraw_num, bs.bowstyle_name, gm.gamemode_name, pa.player_name contestant_a_name, pb.player_name contestant_b_name FROM {$this->table_name} gd
        LEFT JOIN bowstyles bs ON bs.bowstyle_id=gd.bowstyle_id
        LEFT JOIN gamemode gm ON gm.gamemode_id=gd.gamemode_id
        LEFT JOIN player pa ON pa.player_id=gd.contestant_a_id
        LEFT JOIN player pb ON pb.player_id=gd.contestant_b_id
        WHERE gd.gamemode_id=2
        ORDER BY gamedraw_num DESC";
        ...
        if( $result = $this->connection->query( $query ) ){
            if( $result->num_rows > 0 ) {
                ...
                while($row = $result->fetch_assoc()) {
                    ...
                    $res[$i]['id'] = $row['gamedraw_id'];
                    $res[$i]['num'] = $row['gamedraw_num'];
                    $res[$i]['bowstyle_name'] = $row['bowstyle_name'];
                    $res[$i]['gamemode_name'] = $row['gamemode_name'];
    ...
    public function option_data(){
        ...
        $query =
        "SELECT gd.gamedraw_id, gd.gamedraw_num, ta.team_name contestant_a_name, tb.team_name contestant_b_name FROM {$this->table_name} gd
        LEFT JOIN gamemode gm ON gm.gamemode_id=gd.gamemode_id
        LEFT JOIN team ta ON ta.team_id=gd.contestant_a_id
        LEFT JOIN team tb ON tb.team_id=gd.contestant_b_id
        WHERE gd.gamemode_id=1
        UNION
        SELECT gd.gamedraw_id, gd.gamedraw_num, pa.player_name contestant_a_name, pb.player_name contestant_b_name FROM {$this->table_name} gd
        LEFT JOIN gamemode gm ON gm.gamemode_id=gd.gamemode_id
        LEFT JOIN player pa ON pa.player_id=gd.contestant_a_id
        LEFT JOIN player pb ON pb.player_id=gd.contestant_b_id
        WHERE gd.gamemode_id=2
            ...
            if($result->num_rows > 0){
                ...
                if( $result = $this->connection->query( $query ) ){
                    if($result->num_rows>0){
                        ...
                        while($row = $result->fetch_assoc()) {
                            $res[$i]['draw'] = $row['draw'];
                            $res[$i]['style'] = $row['style'];
                            $res[$i]['gamemode'] = $row['gamemode'];
                            $res[$i]['sets'] = $row['sets'];
                            $res[$i]['player_a'] = $row['player_a'];
                            $res[$i]['player_b'] = $row['player_b'];
                            $res[$i]['score_a'] = $row['score_a'];
                            $res[$i]['score_b'] = $row['score_b'];
