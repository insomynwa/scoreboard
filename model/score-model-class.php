...
class Score_Model_Class {
    ...
    /**
     * Get Data for Scoreboard Form
     * without contestant data & style config
     *
     * @return mixed false | array ['scores']
     */
    public function scoreboard_form_data() {
        ...
        if ($result = $this->connection->query($query)) {
            if ($result->num_rows > 1) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    $res['contestants'][$i]['desc'] = $row['score_desc'];
                    $i++;
                }
                return $res;
            }
